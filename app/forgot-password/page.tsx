...
export default function ForgotPasswordPage() {
    ...
    return (
        <div className="relative min-h-screen flex items-center justify-center bg-white p-6 overflow-hidden selection:bg-black/30">
            {/* Background Layer */}
            ...
            <div className="relative z-10 w-full max-w-lg">
                <div className="text-center mb-10 space-y-4">
                    <div className="inline-flex items-center justify-center w-20 h-20 rounded-3xl bg-slate-50 border border-slate-100 shadow-sm mb-2 group">
                        <Mail className="w-10 h-10 text-black group-hover:scale-110 transition-transform" />
                    </div>
                    <div className="space-y-1">
                        <h1 className="text-4xl font-black text-black tracking-tighter uppercase italic">
                            RESET <span className="text-black font-medium">ACCESS</span>
                        </h1>
                        <p className="text-black font-medium text-sm italic">Initiate protocol to restore node credentials.</p>
                    </div>
                </div>
                ...
                <Card className="bg-white/70 backdrop-blur-2xl border-slate-200 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] rounded-[2.5rem] overflow-hidden relative">
                    ...
                    <CardHeader className="text-center p-8 pb-0">
                        <CardTitle className="text-2xl font-black text-black uppercase tracking-tight">Forgot Password</CardTitle>
                        <CardDescription className="text-black font-medium text-sm italic">Enter your registered email to receive reset instructions.</CardDescription>
                    </CardHeader>
                    ...
                    <CardContent className="p-8 pt-6">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            {message && (
                                <Alert className="bg-slate-50 border-slate-100 text-black rounded-2xl py-3 border italic font-black text-xs uppercase">
                                    <AlertDescription>{message}</AlertDescription>
                                </Alert>
                            )}

                            <div className="space-y-2">
                                <Label htmlFor="email" className="text-[10px] font-black uppercase tracking-widest text-black ml-1">
                                    Gateway ID (Email)
                                </Label>
                                <div className="relative group">
                                    ...
                                    <Input
                                        ...
                                        required
                                        disabled={isLoading}
                                        className="h-14 pl-12 bg-white border-slate-200 rounded-2xl text-black focus:border-black/50 focus:ring-black/20 transition-all font-medium placeholder:text-slate-300"
                                        placeholder="identity_anchor@scbbankiing.com"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="text-center pt-2">
                                <Link href="/login" className="inline-flex items-center text-[10px] font-black uppercase tracking-widest text-black hover:text-black transition-colors">
