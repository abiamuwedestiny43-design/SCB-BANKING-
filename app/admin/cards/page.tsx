  ...
  return (
    <div className="min-h-screen bg-[#F4F6FA] p-4 md:p-8 lg:p-12 pt-20 md:pt-28 space-y-6 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl sm:text-3xl md:text-5xl font-black text-slate-900 tracking-tighter italic uppercase">
            Card <span className="text-orange-600">Inventory</span>
          </h1>
          <p className="text-[10px] sm:text-xs md:text-sm text-slate-400 font-bold uppercase tracking-widest opacity-60">Manage physical and virtual card assets.</p>
        </div>
        <Link
          href="/admin"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {[
          { label: "Total Cards Issued", value: cards.length, accent: "text-slate-900", borderColor: "border-slate-200", bg: "bg-white" },
          ...
          { label: "Security Blocks", value: cards.filter(c => c.status === 'blocked').length, accent: "text-red-500", borderColor: "border-red-200", bg: "bg-red-50/30" },
        ].map((stat, i) => (
        ...
        <div className="flex flex-col sm:flex-row gap-2 md:gap-3">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent className="bg-white border-slate-200 rounded-xl">
              <SelectItem value="all" className="text-[10px] font-black uppercase tracking-widest focus:bg-orange-50">All Assets</SelectItem>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-4 md:p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <h2 className="text-lg md:text-xl font-black text-slate-900 tracking-tighter italic uppercase">Vault Ledger</h2>
            <p className="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-widest mt-1">{filteredCards.length} assets indexed</p>
          </div>
          <div className="px-3 md:px-4 py-1.5 md:py-2 bg-slate-900 rounded-xl md:rounded-2xl flex items-center gap-2">
        ...
        <div className="block md:hidden divide-y divide-slate-100">
          {filteredCards.length > 0 ? (
              ...
              <div key={card._id} className="p-4 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-orange-50 border border-orange-100 flex items-center justify-center text-orange-600 shadow-sm shrink-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] font-mono text-slate-400 tracking-wider">•••• •••• •••• {card.cardNumber.slice(-4)}</p>
                    </div>
                  </div>
                  <div className={cn(
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-50">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] font-black text-slate-700 uppercase">{card.cardType}</span>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      ...
                      className={cn(
                        ...
                        card.status === 'active' ? "bg-red-500 hover:bg-red-600 text-white shadow-lg shadow-red-100" : "bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-100"
                      )}
                    >
            ...
            <div className="p-16 text-center">
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">No assets located</p>
            </div>
          )}
        </div>
                ...
                <tr key={card._id} className="hover:bg-slate-50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3 md:gap-4">
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-orange-50 border border-orange-100 flex items-center justify-center text-orange-600 shadow-sm flex-shrink-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] md:text-xs text-slate-400 truncate max-w-[180px]">{card.userId.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="space-y-1">
                      <p className="text-sm md:text-base font-black text-slate-900 font-mono tracking-widest">•••• •••• {card.cardNumber.slice(-4)}</p>
                      <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">Expires {card.expiryDate}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4 hidden lg:table-cell">
                    <Badge className="bg-slate-900 text-white border-none text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-lg italic">
                      {card.cardType}
                    </Badge>
                  </td>
                  <td className="px-6 py-4">
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">No asset identifiers matched the query</p>
                    </div>
                  </td>
                </tr>
              )}
