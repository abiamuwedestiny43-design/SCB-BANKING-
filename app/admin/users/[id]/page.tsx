...
export default async function UserDetailsPage({ params }: { params: { id: string } }) {
  return (
    <div className="p-4 md:p-10 space-y-10 relative">
      ...
      <Suspense fallback={<div className="text-orange-500 font-black animate-pulse">LOADING USER DATA...</div>}>
...
async function UserDetailsContent({ userId }: { userId: string }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 relative z-10">
        ...
        <Card className="bg-white/[0.03] border-white/5 rounded-[3rem] overflow-hidden">
          <CardHeader className="p-10 border-b border-white/5 bg-white/[0.01]">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex items-center gap-6">
                <div className="w-20 h-20 rounded-[2rem] bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-400 text-4xl font-black overflow-hidden">
                  {user.profileImage ? (
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-2">
                    {user.roles.map((role) => (
                      <Badge key={role} className="bg-orange-500/10 text-orange-500 border-orange-500/20 text-[9px] font-black uppercase tracking-widest">
                        {role}
                      </Badge>
                    ))}
                  </div>
          ...
          <CardContent className="p-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-8">
                <h3 className="text-sm font-black text-orange-400 uppercase tracking-[0.2em] mb-4">Account Details</h3>
                {[
                  ...
                  <div key={i} className="group">
                    ...
                    <p className={`text-sm font-bold text-white ${item.mono ? 'font-mono tracking-widest text-orange-500' : ''}`}>{item.value}</p>
                  </div>
                ))}
              </div>
                  ...
                  {user.transfers.map((transfer) => (
                    <tr key={transfer.id} className="hover:bg-white/[0.02] transition-colors group">
                      <td className="px-10 py-4">
                        <Badge className={`uppercase text-[9px] font-black px-2 py-0.5 rounded-lg ${transfer.txStatus === 'success' ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'}`}>
                          {transfer.txStatus}
                        </Badge>
                      </td>
                      <td className="px-10 py-4">
      ...
      <div className="space-y-8">
        <Card className="bg-gradient-to-br from-[#1e293b] to-[#020617] border-orange-500/20 rounded-[2.5rem] p-10 overflow-hidden relative shadow-3xl">
          <div className="absolute top-[-20%] left-[-20%] w-[60%] h-[60%] bg-orange-500/10 rounded-full blur-[100px]"></div>
          <div className="relative z-10 space-y-8">
            <div className="space-y-1">
              <p className="text-[10px] font-black text-orange-500 uppercase tracking-[0.3em]">Total Balance</p>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 rounded-2xl bg-black/20 border border-white/5">
                  ...
                  <Badge className={user.verified ? "bg-orange-500 text-black font-black" : "bg-red-500 text-white font-black"}>
                ...
                <div className="flex items-center justify-between p-4 rounded-2xl bg-black/20 border border-white/5">
                  ...
                  <Badge className={user.canLocalTransfer ? "bg-orange-500 text-black font-black" : "bg-slate-700 text-slate-400 font-black"}>
                ...
                <div className="flex items-center justify-between p-4 rounded-2xl bg-black/20 border border-white/5">
                  ...
                  <Badge className={user.canInternationalTransfer ? "bg-orange-500 text-black font-black" : "bg-slate-700 text-slate-400 font-black"}>
        ...
        <Card className="bg-white/[0.03] border-white/5 rounded-[2.5rem] p-8 space-y-4 relative">
          ...
          <p className="text-xs text-slate-500 leading-relaxed italic border-l-2 border-orange-500/30 pl-4">
