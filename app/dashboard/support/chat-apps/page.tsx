        ...
        <div className="min-h-screen bg-[#F4F6FA] p-4 md:p-6 pt-16 lg:pt-6">
            <div className="max-w-4xl mx-auto space-y-5">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-base font-bold text-black">Support Channels</h1>
                        <p className="text-xs text-black">Connect with our support team</p>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                        ...
                        <div className="p-4 bg-slate-50/50 flex items-center gap-4">
                            <div className="flex items-center gap-2">
                                <div className="h-6 w-6 rounded-lg bg-slate-100 flex items-center justify-center text-black">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {chatApps.map((app, i) => (
                        <motion.div key={i} {...fadeIn} transition={{ delay: 0.1 * i }}>
                            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 hover:border-slate-200 transition-all group h-full flex flex-col">
                                <div className="flex justify-between items-start mb-3">
                                    <div className="h-9 w-9 rounded-lg bg-slate-50 flex items-center justify-center text-black group-hover:bg-slate-50 group-hover:text-black transition-colors">
                                        <MessageCircle className="h-5 w-5" />
                                    </div>
                                    {app.status && (
                                        <Badge className="bg-slate-50 text-black hover:bg-slate-100 border-none px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider">
                                            {app.status}
                                        </Badge>
                                    )}
                                </div>
                                ...
                                <div className="mb-3">
                                    <h3 className="text-sm font-bold text-black group-hover:text-black transition-colors">{app.name}</h3>
                                    <p className="text-[10px] text-black font-semibold uppercase tracking-wider">{app.category}</p>
                                </div>

                                <p className="text-xs text-black mb-4 flex-grow">{app.description}</p>

                                <div className="space-y-1.5 mb-4">
                                    {app.features.map((feature, idx) => (
                                        <div key={idx} className="flex items-center gap-2 text-[10px] font-medium text-black">
                                ...
                                <Button size="sm" className="w-full bg-black hover:bg-black text-white rounded-lg font-semibold text-xs h-9 transition-all">
                    ...
                    <div className="bg-black rounded-xl p-6 text-white relative overflow-hidden">
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-xs text-black max-w-xl">Never share your password, OTP, or PIN on any support channel. Official SCB BANKING staff will never ask for these details.</p>
                            </div>
                ...
                <div className="text-center pb-6">
                    <p className="text-[10px] font-bold text-black uppercase tracking-widest flex items-center justify-center gap-2">
                        <HelpCircle className="h-3.5 w-3.5" />
                        Need immediate assistance?
                        <Link href="/dashboard/support" className="text-black hover:underline">Contact Support</Link>
                    </p>
                </div>
            </div>
        </div>
