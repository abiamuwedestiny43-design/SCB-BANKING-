  ...
  const markAllRead = async () => {
    ...
    if (res.ok) {
      toast({
        ...
        className: "bg-indigo-600 border-none text-white font-bold"
      })
      mutate()
    }
  }
        ...
        <motion.div {...fadeInUp} className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-white/5">
          ...
          <div className="flex items-center gap-3">
            ...
            {unreadCount > 0 && (
              <Button
                onClick={markAllRead}
                className="h-12 px-6 bg-indigo-500 hover:bg-indigo-400 text-[#020617] rounded-xl shadow-xl shadow-indigo-500/20 font-black flex items-center gap-2 transition-all hover:-translate-y-1"
              >
        ...
        <motion.div {...fadeInUp} transition={{ delay: 0.1 }} className="space-y-6">
          <div className="flex items-center justify-between px-2">
            <h2 className="text-lg font-black text-white lowercase tracking-tight flex items-center gap-3">
              ...
              <span className="text-[10px] font-black bg-indigo-500/10 border border-indigo-500/20 text-indigo-500 px-3 py-1 rounded-full uppercase tracking-widest animate-pulse">{unreadCount} Pending</span>
            </h2>
          </div>
              ...
              {isLoading ? (
                <div className="py-24 text-center space-y-6">
                  <div className="relative h-20 w-20 mx-auto">
                    <div className="absolute inset-0 bg-indigo-500/20 rounded-[2rem] animate-ping"></div>
                    <div className="relative h-20 w-20 bg-white/5 border border-white/10 rounded-[2rem] flex items-center justify-center text-indigo-500">
                      ...
                      return (
                        <motion.div
                          ...
                          className={cn(
                            ...
                            !n.viewed && "bg-indigo-500/[0.03]"
                          )}
                        >
                          {/* Unread Indicator Vertical Bar */}
                          {!n.viewed && (
                            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-16 bg-indigo-500 rounded-r-full shadow-[0_0_20px_rgba(16,185,129,0.5)]"></div>
                          )}
                          ...
                          <div className={cn(
                            ...
                            isDebit ? "bg-red-500/10 border-red-500/20 text-red-500" :
                              isCredit ? "bg-indigo-500/10 border-indigo-500/20 text-indigo-500" :
                          ...
                          <div className="flex-1 space-y-4">
                              ...
                              <div className="flex items-center gap-3">
                                <span className={cn(
                                  ...
                                  isDebit ? "bg-red-500/10 text-red-400 border-red-500/20" :
                                    isCredit ? "bg-indigo-500/10 text-indigo-400 border-indigo-500/20" :
                                      "bg-blue-500/10 text-blue-400 border-blue-500/20"
                                )}>
                                  {isDebit ? "Egress Alert" : isCredit ? "Ingress Alert" : "Log Entry"}
                                </span>
                                {!n.viewed && (
                                  <div className="h-2 w-2 bg-indigo-500 rounded-full shadow-[0_0_8px_rgb(16,185,129)] animate-pulse" />
                                )}
                              </div>
                            ...
                            <div className="relative">
                              ...
                              <div className="absolute -bottom-1 right-0 opacity-10 blur-xl bg-indigo-500 h-8 w-32 group-hover:opacity-20 transition-opacity"></div>
                            </div>

                            {n.redirect && (
                              <Link
                                href={n.redirect}
                                className="inline-flex items-center gap-2 text-[10px] font-black text-indigo-500 hover:text-indigo-400 uppercase tracking-[0.2em] pt-2 group/link"
                              >
                                Access Source
